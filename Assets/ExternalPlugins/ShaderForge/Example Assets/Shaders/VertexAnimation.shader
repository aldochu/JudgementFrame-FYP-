    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_833 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_833.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_833 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_833.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_133 = pow((abs((frac((o.uv0+node_8196.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(_Object2World, v.vertex);
                float3 lightColor = _LightColor0.rgb;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 _Normals_var = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(i.uv0, _Normals)));
                float4 node_8196 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_8196.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float3 normalLocal = normalize(lerp(_Normals_var.rgb,float3(0,0,1),node_133));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv2 = v.texcoord2;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_5701 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_5701.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(_Object2World, v.vertex);
                o.pos = mul(UNITY_MATRIX_MVP, v.vertex );
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv2 = v.texcoord2;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_1155 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((o.uv0+node_1155.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(_Object2World, v.vertex);
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 node_1155 = _Time + _TimeEditor;
                float node_133 = pow((abs((frac((i.uv0+node_1155.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float node_1924 = node_133;
