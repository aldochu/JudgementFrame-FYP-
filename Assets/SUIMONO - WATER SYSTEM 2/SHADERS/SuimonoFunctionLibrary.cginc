	...
	sampler2D _HeightTexD;
	sampler2D _NormalTexD;
	sampler2D _HeightTexR;
	...
	fixed4 screenTransparent;
	fixed4 screenTransparentDistort;
	fixed4 screenCaustics;
	fixed4 screenCausticsDistort;
	fixed4 screenDepthMask;
	fixed4 screenDepthMaskDistort;
	...
	//vertex setup
		...
		//Calm Wave Height
		float CheightPos0 = tex2Dlod(_HeightTex, float4(texUV + flowSpeed + waveOff1,0,0)).r;
		float CheightPos1 = tex2Dlod(_HeightTex, float4(texUV * 0.75 - (flowSpeed*0.25) + (waveOff1*0.75),0,0)).r;

		//Turbulent Wave Height
		float TheightPos0 = tex2Dlod(_HeightTexD, float4(texUV + flowSpeed + waveOff1,0,0)).r;
		float TheightPos1 = tex2Dlod(_HeightTexD, float4(texUV * 0.5 - (flowSpeed*0.25) + (waveOff1*0.5),0,0)).r;

		//Rolling Wave Height
		float RheightPos0 = tex2Dlod(_HeightTexR, float4(texUV * _lgWaveScale + (flowSpeed*0.0625) + (waveOff1*_lgWaveScale),0,0)).r;
		float RheightPos1 = tex2Dlod(_HeightTexR, float4(texUV * _lgWaveScale + (flowSpeed*0.125) + (waveOff1*_lgWaveScale),0,0)).r;
		...
		useY = lerp(0.0,useY,_heightProjection);
			...
			//shoreline waves
			useY += (shoreFac*(1.8-shoreTex.g)*10.0);
		...
		//blend wave vertice offset in distance
	 	...
	 	v.vertex.y = v.vertex.y + (useY * lerp(0.464646,1.0,_Suimono_isLinear));
	...
	half3 SuimonoCalcNormal(float2 uvs, float2 uvWave, float4 uvScreen){
		...
		fixed4 CheightTex0 = tex2D(_HeightTex, uvs + flowSpeed + waveOff1);
		fixed4 CheightTex1 = tex2D(_HeightTex, uvs * 0.75 - (flowSpeed*0.25) + (waveOff1*0.75));
		half3 CnormalTex0 = lerp(half3(0,0,1),UnpackNormal( tex2D(_NormalTex, uvs + flowSpeed + waveOff1)),_NormalStrength*_heightScale);
		half3 CnormalTex1 = lerp(half3(0,0,1),UnpackNormal( tex2D(_NormalTex, uvs * 0.75 - (flowSpeed*0.25) + (waveOff1*0.75))),_NormalStrength*_heightScale);
		half cHeight = (CheightTex0.r + CheightTex1.r) * 0.8;
		half3 cNormal = float3(CnormalTex0.xy + CnormalTex1.xy, CnormalTex0.z * CnormalTex1.z);
			nFac = saturate((0.0+lerp(1.0,0.0,uvScreen.w/400)));
			cNormal = lerp(half3(0,0,1),cNormal,nFac);

		//Turbulent Wave Normals and Height
		fixed4 TheightTex0 = tex2D(_HeightTexD, uvs + flowSpeed + (waveOff1));
		fixed4 TheightTex1 = tex2D(_HeightTexD, uvs * 0.5 - (flowSpeed*0.25) + (waveOff1*0.5));
		half3 TnormalTex0 = lerp(half3(0,0,1),UnpackNormal( tex2D(_NormalTexD, uvs + flowSpeed + (waveOff1)) ),_NormalStrength);
		half3 TnormalTex1 = lerp(half3(0,0,1),UnpackNormal( tex2D(_NormalTexD, uvs * 0.5 - (flowSpeed*0.25) + (waveOff1*0.5)) ),_NormalStrength);

		half tHeight = (TheightTex0.r*0.2) + (TheightTex0.r * TheightTex1.r * 0.8) * _turbulenceFactor * 0.5;
		half3 tNormal  = float3(TnormalTex0.xy + TnormalTex1.xy, TnormalTex0.z * TnormalTex1.z);
		half3 tbNormal = tNormal;

		half3 TnormalTex3 = lerp(half3(0,0,1),UnpackNormal( tex2D(_NormalTexD, uvs * 6.0 + (flowSpeed*0.5) + (waveOff1*6.0) + (tNormal.xy * 0.06 * lerp(0.0,1.0,tNormal.z))) ),_NormalStrength);
		tNormal = lerp(tNormal,float3(tNormal.xy + TnormalTex3.xy, tNormal.z * TnormalTex3.z),saturate(lerp(0.0,1.0,_turbulenceFactor)));
			nFac = saturate((0.0+lerp(2.0,0.0,uvScreen.w/4000)));
			tNormal = lerp(half3(0,0,1),tNormal,nFac);

		//Rolling Wave Normals and Height
		fixed4 RheightTex0 = tex2D(_HeightTexR, uvs * _lgWaveScale + (flowSpeed*0.0625) + (waveOff1*_lgWaveScale));
		fixed4 RheightTex1 = tex2D(_HeightTexR, uvs * _lgWaveScale + (flowSpeed*0.125) + (waveOff1*_lgWaveScale));
		half3 RnormalTex0 = lerp(half3(0,0,1),UnpackNormal( tex2D(_NormalTexR, uvs * _lgWaveScale + (flowSpeed*0.0625) + (waveOff1*_lgWaveScale))),_NormalStrength);
		half3 RnormalTex1 = lerp(half3(0,0,1),UnpackNormal( tex2D(_NormalTexR, uvs * _lgWaveScale + (flowSpeed*0.125) + (waveOff1*_lgWaveScale))),_NormalStrength);
			

		half rHeight = ((RheightTex0.r * 4.0) + (RheightTex1.r*3.0));
		half3 rNormal = float3(RnormalTex0.xy + RnormalTex1.xy, RnormalTex0.z * RnormalTex1.z);
			nFac = saturate((0.0+lerp(1.0,0.0,uvScreen.w/6000)));
			rNormal = lerp(half3(0,0,1),rNormal,nFac);
		...
		heightMask = (cHeight * _heightScale);
		heightMask += (tHeight * _heightScale);
		heightMask += (rHeight * _lgWaveHeight);
		...
		//half3 flowWaves = tex2D(_WaveTex,(float2(2.5,uvWave.y)*flowmap + waveOff1)-float2(_Time.x*_shorelineSpeed,1.0)).rgb * saturate(screenShoreline.r*0.5);
		//half3 flowWaves = tex2D(_WaveTex,(float2(2.5,uvWave.y)*flowmap)-float2(_Time.x*_shorelineSpeed,1.0)).rgb * saturate(screenShoreline.r*0.5);
		//half3 flowWaves = tex2D(_WaveTex,(float2(2.5,uvWave.y)*flowmap)).rgb;
		tempWaves = saturate(lerp(flowWaves.r,flowWaves.g,screenShoreline.g)) * screenShoreline.r;
		heightMask += tempWaves.r * _shorelineHeight;
	...
	half4 LightingSuimonoLight (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten){
		...
		//Under Scattering
		col.rgb = lerp(col.rgb, _SSSColor.rgb * outLight.rgb * atten * 2, saturate(lerp(0.4,0.2,dot(viewDir,lightDir)+1.8))*_SSSColor.a);
		...
		refTerm = refTerm * saturate(fresnel*lerp(0.4,1,atten)*refFac);
		...
		//combine rough specular
		col.rgb = col.rgb + (lerp(((saturate(ndf2 * h * lerp(1.0,0.1,_roughness)) * atten) * _SpecularColor.rgb * outLight.a),0.0,edgeDepth)*_SpecularColor.a*1.0);
	...
	//LIGHTING FOAM --------------------------------------------------------------------------------------------------------------
	half4 LightingSuimonoLightFoam (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten){
		...
		half fresnel;
		fresnel = _reflecTerm+(1.0-_reflecTerm)*pow((dot(s.Normal,normalize(lightDir+viewDir))),5);
		fresnel = fresnel * (_reflecTerm+(1.0-_reflecTerm)*pow((1.0-cNdotV),5));
		fresnel = saturate(max(fresnel,_reflecTerm+(1.0-_reflecTerm)*pow((1.0-cNdotV),5)));

		//-------------------------------------
		//##  GEOMETRY FUNCTIONS (Implicit)  ##
		//-------------------------------------
		half gf = max(0,dot(s.Normal,lightDir))*cNdotV;

		fixed4 col;
		col.rgb = _depthColor.rgb * outLight.rgb * _overallBrightness;
		col.a = 0.0;

		//combine foam
		#if defined(SUIMONO_FOAM_ON)

			foamDepth = saturate(foamDepth);
			col.a = saturate(foamDepth * _FoamColor.a);

			col.rgb = lerp(_FoamColor.rgb, _FoamColor.rgb * outLight.rgb, lerp(0.2,0.9,gf)) * lerp(0.05,1.2,outLight.a);

			//add direct light casting
			col.rgb = col.rgb - lerp(0.1,0,gf);
			col.rgb = saturate(col.rgb);

			//combine extreme edge darken
			col.rgb = lerp(col.rgb,col.rgb * 0.0,edgeDarkenDepth*0.4*_FoamColor.a);
			col.rgb = col.rgb * lerp(1.0,0.4,saturate(foamDarkenDepth)*_FoamColor.a);

		#endif

		//atten
		col.rgb = saturate(col.rgb * atten);

		//combine bright specular
		half spc = (saturate(ndf * h * lerp(1.0,0.1,_roughness)) * atten);
		col.rgb = col.rgb + (spc * outLight.rgb)*1.75;

		//fresnel reduction
		col.a = col.a * lerp(1.0,0.8,fresnel-(spc*1.75));
...
//SURFACE --------------------------------------------------------------------------------------------------------------
    void SuimonoSurf (Input IN, inout SurfaceOutput o) {
		...
		uv0 = float4(max(0.001f, uv0.x),max(0.001f, uv0.y),max(0.001f, uv0.z),max(0.001f, uv0.w));
		o.Normal = SuimonoCalcNormal(IN.uv_HeightTex, IN.uv3_WaveTex.y, uv0);
		...
		uv1.y = uv1.y - (1.0 * o.Normal.y *_RefractStrength);
		#if defined(SUIMONO_TRANS_ON)
		...
		float sceneZ2 = 0;
		...
		#if SHADER_API_D3D11
			//DX11
			sceneZ1 = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(uv0)).r);
			sceneZ2 = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(uv1)).r);

			depthDifferential = ((screenTransparentDistort.a-0.0001)-((sceneZ2+0.0055)/500));
			foamDifferential = ((screenTransparent.a)-(sceneZ1/500));
			...
			depthDifferential = lerp(foamDifferential,((screenTransparentDistort.a)-(sceneZ2/500)),_distortMask);
			edgeDepth = saturate(lerp(1.0,-10.0,_EdgeFade * depthDifferential));
			edgeDarkenDepth = saturate(lerp(1.0,-10.0,800 * depthDifferential));
			half foamSpread = saturate(lerp(1.0,-20.0,_EdgeFoamFade * depthDifferential)*2);
...
//SURFACE FOAM--------------------------------------------------------------------------------------------------------------
void SuimonoSurfFoam (Input IN, inout SurfaceOutput o) {

		//get normal data
		float4 uv0 = IN.screenPos;
		uv0 = float4(max(0.001f, uv0.x),max(0.001f, uv0.y),max(0.001f, uv0.z),max(0.001f, uv0.w));
		o.Normal = SuimonoCalcNormal(IN.uv_HeightTex, IN.uv3_WaveTex.y, IN.screenPos);

		//get screen Textures ------------------------------------------------------------------------------------------------------
		float4 uv6 = IN.screenPos;
		uv6 = float4(max(0.001f, uv6.x),max(0.001f, uv6.y),max(0.001f, uv6.z),max(0.001f, uv6.w));
		uv6.y = uv6.y - 1 * (heightMask + heightMaskRWave) * _heightScale;
		screenNormals = tex2Dproj(_suimono_NormalsTex, UNITY_PROJ_COORD(uv6));
		#if SHADER_API_D3D11
			screenWakeMask = tex2Dproj(_suimono_WakeTex, UNITY_PROJ_COORD(uv6));
		#else
			screenWakeMask = fixed4(0,0,0,1);
		#endif
		...
		#if defined(SUIMONO_TRANS_ON)
			screenTransparent = tex2Dproj(_suimono_TransTex, UNITY_PROJ_COORD(uv0));
			screenTransparentDistort = tex2Dproj(_suimono_TransTex, UNITY_PROJ_COORD(uv1));
		#else
			screenTransparent = half4(_depthColor.r,_depthColor.g,_depthColor.b,1.0);
			screenTransparentDistort = half4(_depthColor.r,_depthColor.g,_depthColor.b,1.0);
		#endif
		...
		//calculate water depth masks ---------------------------------------------------------------------------------------------
		#if SHADER_API_D3D11
			//DX11
			float sceneZ1 = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(uv0)).r);
			float sceneZ2 = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(uv1)).r);
			depthDifferential = ((screenTransparentDistort.a-0.0001)-((sceneZ2+0.0055)/500));
			foamDifferential = ((screenTransparent.a)-(sceneZ1/500));
			...
			depthDifferential = lerp(foamDifferential,((screenTransparentDistort.a)-(sceneZ2/500)),_distortMask);
			edgeDepth = saturate(lerp(1.0,-10.0,0.0 * depthDifferential));
			edgeDarkenDepth = saturate(lerp(1.0,-10.0,800 * depthDifferential));
			half foamSpread = saturate(lerp(1.0,-20.0,_EdgeFoamFade * depthDifferential)*2);
			foamSpread = foamSpread - saturate(lerp(1.0,-10.0,1000.0 * depthDifferential));
		#else
			//DX9
			float sceneZ1 = Linear01Depth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(uv0)).r);
			float sceneZ2 = Linear01Depth(SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(uv1)).r);
			depthDifferential = ((screenTransparentDistort.a+0.0001)-((sceneZ2-0.0003)));
			foamDifferential = ((screenTransparent.a)-(sceneZ1));
			...
			depthDifferential = lerp(foamDifferential,((screenTransparentDistort.a)-(sceneZ2)),_distortMask);
			edgeDepth = saturate(lerp(1.0,-22.0,0.0 * depthDifferential));
			edgeDarkenDepth = saturate(lerp(1.0,-22.0,800 * depthDifferential));
			half foamSpread = saturate(lerp(1.0,-40.0,_EdgeFoamFade * depthDifferential)*2);
			foamSpread = foamSpread - saturate(lerp(1.0,-2.0,1000.0 * depthDifferential));
		#endif
		...
		//CALCULATE FOAM ----------------------------------------------------------------------------------------------------------
			...
			float2 flowSpeed = float2(_suimono_Dir.x,_suimono_Dir.z)*_suimono_Dir.w;
			float2 waveOff1 = float2(_suimono_uvx,_suimono_uvy);
			fixed4 foamTex = tex2D(_FoamTex, (IN.uv_HeightTex * _foamScale) + (flowSpeed*_foamSpeed) + (waveOff1 * _foamScale) - (tbNormal.xy * 5 * tbNormal.z));
			fixed4 foamMask = tex2D(_FoamTex, (IN.uv_HeightTex * _foamScale * 0.0625) + (flowSpeed*_foamSpeed*0.0625) + (waveOff1 * _foamScale * 0.0625) - (tbNormal.xy * 5 * tbNormal.z));
